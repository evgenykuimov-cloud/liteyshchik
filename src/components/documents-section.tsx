...
export function DocumentsSection() {
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{result.map((item) => <article key={item.id} className="border border-[var(--border)] bg-[var(--surface)] p-6"><FileClock className="text-[var(--accent)]" /><p className="mt-8 text-xs uppercase text-[var(--foreground-muted)]">{item.type} · {item.category}</p><h2 className="heading mt-3 text-3xl">{item.title}</h2><p className="mt-5 border-t border-[var(--border)] pt-4 text-sm text-[var(--foreground-muted)]">TODO: файл ожидает загрузки и проверки</p></article>)}</div>
    </div>
  );
}
